...
def pos_neg_value(sh,s,i):
    ...
    for w in words:
        ...
        try:
            ...
            neg = a[0].neg_score()
            print w
            print "Positiveness: ",pos
            print "Negativeness: ",neg
        except:
            pos = 0.0
            neg = 0.0
            print "error\n"
        if pos>neg:
            print w," is a positive word."
            pw = pw+1
        elif neg>pos:
            print w," is a negative word."
            nw = nw+1
        else:
            print w," is a neutral word."
            ntw = ntw+1
...
for i in range(len(total_queries)):
    ...
    if(len(common_nouns)!=len(main_query_nouns)):
        for w in q_nouns:
            try:
                meaning = wikipedia.summary(w,sentences=2)
            except wikipedia.exceptions.DisambiguationError:
                meaning = 'many_meaning'
...
for i in range(len(total_queries)):
    ...
    for x in uncommon:
        try:
            meaning = wikipedia.summary(x,sentences=2)
        except wikipedia.exceptions.DisambiguationError:
            meaning = 'many_meaning'
        except wikipedia.exceptions.PageError:
            meaning = 'many_meaning'
...
shl1 = wb.sheet_by_index(0)
relevancy = shl1.col_values(1)
wb = Workbook()
sh = wb.add_sheet('Sheet1')
sh.col(0).widh = 17000
#sentiment = sh1.col_values(2)
i=0
for s in total_queries:
    ...
    sh.write(i,0,s)
    for w,v in dictionary.iteritems():
        if w in word:
            val = computeTF(w,s)
            idf = dictionary[w]
            ...
            dictionary[w] = 0
    mod = modulus(dictionary)
    sh.write(i,1,mod)
    pos_neg_value(sh,s,i)
    flag = 0
    sh.write(i,5,relevancy[index])
    #sh.write(i,6,sentiment[index])    
    i = i+1    
    wb.save('Features_l2.xls')
wb.save('Features_l2.xls')
#--------------------------------------------------------------------------------------
#LEVEL2 CLASSIFIER
#--------------------------------------------------------------------------------------
if inp0=='ecig':
    inp2 = 'ecig_l2_trainF.xls'
elif inp0=='skincancer':
    inp2 = 'skincancer_l2_trainF.xls'
elif inp0=='hrt':
    inp2 = 'hrt_l2_trainF.xls'
elif inp0=='vitaminC':
    inp2 = 'vitaminc_l2_trainF.xls'
elif inp0=='mmr_vaccine':
    inp2 = 'mmr_vaccine_l2_trainF.xls'    
data_file_l2 = inp2
wb2 = xlrd.open_workbook(data_file_l2)
sh2 = wb2.sheet_by_index(0)
target_l2 = sh2.col_values(6)
row_num = len(sh2.col_values(0))
n_features_l2 = [sh2.col_values(1),sh2.col_values(2),sh2.col_values(3),sh2.col_values(4),sh2.col_values(5)]
n_features_l2 = np.array([n_features_l2])
#n_features[n_features == ''] = 0.0
...
d2_n_features_l2 = d2_n_features_l2.reshape((row_num,5))

#inp_l2t = raw_input("Enter Test data for 2nd Label:")
data_file_l2t = 'Features_l2.xls'
wb2 = xlrd.open_workbook(data_file_l2t)
sh2 = wb2.sheet_by_index(0)
#target_l2t = sh2.col_values(6)
row_num = len(sh2.col_values(0))
n_features_l2t = [sh2.col_values(1),sh2.col_values(2),sh2.col_values(3),sh2.col_values(4),sh2.col_values(5)]
...
#print d2_n_features_l2t
d2_n_features_l2t = d2_n_features_l2t.reshape((row_num,5))
...
#s2 = classifier2.score(d2_n_features_l2,target_l2)
#print "prediction,s2",s2
wb12 = Workbook()
